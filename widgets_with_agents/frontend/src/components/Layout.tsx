...
export function Layout({ children }: LayoutProps) {
  ...
  return (
    <div className="min-h-screen flex flex-col relative">
      <header className="sticky top-0 z-20 backdrop-blur-xl bg-white/80 border-b border-slate-200/90 shadow-sm">
        <div className="max-w-6xl mx-auto px-3 sm:px-4 h-12 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            {profile?.uid && (
              <span className="text-sm text-slate-500 font-mono bg-slate-100/80 px-2 py-0.5 rounded">{profile.uid}</span>
            )}
      ...
      <main className="flex-1 max-w-6xl w-full mx-auto px-3 sm:px-4 py-4 sm:py-5 relative z-10">
        {children ?? <Outlet />}
      </main>
    </div>
  );
